...
2. Use [this template](https://github.com/CommonGateway/ZGWZdsBundle/generate)
...
5. Update file names and namespace to your fitting 
   ...
   - Open ZgwZdsBundle.php and change the Bundle name and namespace. The namespace should be the same as your package name in composer.json but in CamelCase. So common-gateway/zgw-zds-bundle becomes CommonGateway/ZGWZdsBundle
   - Rename the /Service and /ActionHandler accordingly (or delete if not used).
   - Rename the /DependencyInjection/ZgwZdsExtension.php to your BundleNameExtension.php
...
2. Open a php file where you want to use a class.
   - Add the correct use statement (example `use CommonGateway\ZgwZdsBundle\Service\ZgwZdsService;`)
   - U can now use your class!

In the common gateway, if you want to use your code when triggered by an event with a action, make sure the class of the action object is set as the handler name including the namespace. For example if I want to use the ZgwZdsService I can set the ZgwZdsHandler as `CommonGateway\ZgwZdsBundle\ActionHandler\ZgwZdsHandler`.
